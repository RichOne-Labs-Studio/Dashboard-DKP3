...
let miderDemoTimer = null;
let miderDemoStep = 0;

function startMiderDemo(){

  document.documentElement.requestFullscreen();

  // document.getElementById('demoControl').style.display='block';

  if(miderDemoTimer){
  clearInterval(miderDemoTimer);
}

  const demoFlow = [

    () => {
      showLayer('dashboardLayer', true);
      resetFilters();
    },

    () => {
      pilihUrusanSidebar('Ketahanan Pangan');
    },

    () => {
      pilihUrusanSidebar('Pertanian');
    },

    () => {
      pilihUrusanSidebar('Perikanan');
    },

    () => {
      pilihUrusanSidebar('Peternakan');
    },

    () => {
      showLayer('mapLayer', true);
    }
  ...
  miderDemoTimer = setInterval(() => {

    miderDemoStep++;

    if(miderDemoStep >= demoFlow.length){
      miderDemoStep = 0;
    }

    demoFlow[miderDemoStep]();

  },15000);

}

function stopMiderDemo(){
  ...
  // document.getElementById('demoControl').style.display='none';
